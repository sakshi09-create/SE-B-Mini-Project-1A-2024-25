...
String value3 = txt_dep.getText(); // Use the correct field for department
...
if (dialogResult == JFileChooser.APPROVE_OPTION) {
    ...
    try {
        // Retrieve deductions
        String sqlDeductions = "SELECT * FROM Deductions WHERE emp_id = ?";
        pst = conn.prepareStatement(sqlDeductions);
        pst.setString(1, value1);
        rs = pst.executeQuery();

        String val = "0"; // Default value if no deductions found
        String reason = "N/A"; // Default reason if no deductions found
        ...
        // Close ResultSet before opening another
        rs.close();
        pst.close();

        // Retrieve allowances
        String sqlAllowances = "SELECT * FROM Allowance WHERE emp_id = ?";
        pst = conn.prepareStatement(sqlAllowances);
        pst.setString(1, value1);
        rs = pst.executeQuery();

        int calcTotal = Integer.parseInt(txt_salary.getText());
        float x = 0; // Default value if no allowances found
        ...
        myDocument.add(new Paragraph("Total Earnings: Rs. " + x, FontFactory.getFont(FontFactory.TIMES_ROMAN, 10, Font.PLAIN)));
    ...
    java.awt.EventQueue.invokeLater(() -> new searchEmpSalarySlip().setVisible(true));
}
